...
export function BrandUpdatesSection({ accountId, canPost }: BrandUpdatesSectionProps) {
  const queryClient = useQueryClient();
  const [content, setContent] = useState('');
  const [error, setError] = useState('');

  const { data, isLoading } = useQuery({
    queryKey: ['brand-updates', accountId],
    queryFn: () => apiFetch<{ data: BrandUpdate[] }>(`/brands/${accountId}/updates`),
    enabled: !!accountId,
  });

  const postMutation = useMutation({
    mutationFn: (text: string) =>
      apiFetch(`/brands/${accountId}/updates`, {
        method: 'POST',
        body: JSON.stringify({ content: text }),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['brand-updates', accountId] });
      setContent('');
      setError('');
    },
    ...
    <section className="pt-4 border-t border-border/40">
      ...
      {canPost && (
        <form onSubmit={handlePost} className="mb-4 space-y-2">
          <textarea
            ...
            rows={3}
            placeholder="Share a pipeline update for managers and the team..."
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground italic">
          No updates yet. Post the latest brand progress for your manager and team.
        </p>
      ) : (
